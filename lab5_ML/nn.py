...
def simple_deep_learning_model(X_train, y_train, X_test, y_test, X_all, shape_in):
    ...
    model.fit(X_train, y_train, nb_epoch=500, batch_size=5, verbose=1)
    print("training time:", round(time() - t0, 3), "s")
...
#model_DTree, score_DTree = DTree(X_train, y_train_ohe, X_test, y_test_ohe)
...
output_prediction(model_deep_learning)
